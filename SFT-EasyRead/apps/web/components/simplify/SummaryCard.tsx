"use client"

import { Card, SectionTitle } from "@/components/shared/ui"
import { IconClipboard, IconArrow } from "@/components/shared/icons"

export function SummaryCard({
  title,
  points,
  done,
  onCopy,
}: {
  title: string
  points: string[]
  done: boolean
  onCopy?: () => void
}) {
  return (
    <Card className="bg-brand-soft">
      <SectionTitle
        icon={<IconClipboard width={18} height={18} />}
        title="Ringkasan Otomatis"
        action={<span className="text-sm text-ink-soft">Bacaan: {title}</span>}
      />
      {done ? (
        <ul className="space-y-2">
          {points.map((point) => (
            <li key={point} className="flex gap-3 font-dyslexic text-ink">
              <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-brand" />
              {point}
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-sm text-ink-soft">Ringkasan ide utama akan muncul di sini setelah teks disederhanakan.</p>
      )}
      {done && (
        <button
          type="button"
          onClick={onCopy}
          className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-brand-strong hover:underline-offset-2"
        >
          Salin Ringkasan <IconArrow width={15} height={15} />
        </button>
      )}
    </Card>
  )
}
